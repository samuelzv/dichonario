...
def main():
    ...
    if option == "1":
        ...
        result = run(["docker", "exec", "-it", "-u", "root", "dichonario-app-1", "/bin/sh", "-c" , "python manage.py makemessages -l es"])
    elif option == "2":
        print("Compiling messages...")
        result = run(["docker", "exec", "-it", "-u", "root", "dichonario-app-1", "/bin/sh", "-c" , "python manage.py compilemessages"])
    elif option == "3":
        ...
        result = run(["docker-compose", "-f", "docker-compose-deploy.yml", "down", "app"], capture_output=True, encoding="utf-8")
    elif option == "8":
        print("Update Web App - Build App...")
        result = run(["docker-compose", "-f", "docker-compose-deploy.yml", "build", "app"], capture_output=True, encoding="utf-8")
    elif option == "9":
        print("Update Web App - Up App...")
        result = run(["docker-compose", "-f", "docker-compose-deploy.yml", "up", "app", "--detach", "--no-recreate"], capture_output=True, encoding="utf-8")
